...
def draw_pie_at(
    ...
    ax.add_patch(Circle((x, y), r * 0.06, facecolor=colors[0], edgecolor="none"))
...
def draw_circled_number(ax: plt.Axes, x: float, y: float, number: int) -> None:
    ...
    ax.add_patch(circ)
    ax.text(x, y, str(number), ha="center", va="center", fontsize=14, color=BLACK)
...
def draw_proxy_box(ax, x, y, w: float = 2.4, h: float = 1.2):
    ...
    ax.add_patch(box)
    ax.text(x, y, "proxy", ha="center", va="center", fontsize=13, color=BLACK)
    return (x - w / 2, x + w / 2)


def panel_titles(ax: plt.Axes) -> None:
    draw_circled_number(ax, 1.0, 8.5, 1)
    ax.text(2.2, 8.5, "Swarm", ha="left", va="center", fontsize=18, color=BLACK)
...
def draw_swarm_panel(ax: plt.Axes) -> None:
    ...
    ax.text(1.15, 3.8, r"$\vdots$", ha="center", va="center", fontsize=18, color=BLACK)
...
def draw_regression_panel(ax: plt.Axes) -> None:
    ...
    ax.plot([x0, x0], [y0, y0 + h], color=BLACK, lw=2.5)
    ax.plot([x0, x0 + w], [y0, y0], color=BLACK, lw=2.5)

    ax.plot([x0 + 0.6, x0 + w - 0.6], [y0 + 0.6, y0 + h - 0.6], color=FHAT_COLOR, lw=2.5)
    ...
    for px, py in pts:
        ax.plot(x0 + px, y0 + py, marker=".", markersize=6, color=Y_COLOR, lw=0)

    ax.text(x0 + w / 2, y0 - 1.0, r"Predicted $\hat{f}(p)$", ha="center", va="center", fontsize=16, color=FHAT_COLOR)
    ax.text(x0 - 1.0, y0 + h / 2, r"True $f(p)$", ha="center", va="center", rotation=90, fontsize=16, color=Y_COLOR)
...
def draw_math_segments(ax, x, y, segments, fontsize=20, va="center"):
    ...
    cur_x = x
    for s, c in segments:
        t = ax.text(cur_x, y, s, ha="left", va=va, fontsize=fontsize, color=c)
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=Path("fig_sro.pdf"))
