...
def draw_pie_at(
    ...
    ax.add_patch(Circle((x, y), r * 0.06, facecolor=colors[0], edgecolor="none", path_effects=[
            pe.withStroke(linewidth=3.2, foreground=BACKGROUND),
            pe.Normal(),
        ],))
    ...
    ax.add_patch(
        Circle(
            ...
            linewidth=outline_lw, path_effects=[
            pe.withStroke(linewidth=3.2, foreground=BACKGROUND),
            pe.Normal(),
        ],
        )
...
def draw_circled_number(ax: plt.Axes, x: float, y: float, number: int) -> None:
    circ = Circle((x, y), 0.8, fill=False, edgecolor=BLACK, linewidth=2, path_effects=[
            pe.withStroke(linewidth=3.2, foreground=BACKGROUND),
            pe.Normal(),
        ],)
    ax.add_patch(circ)
    ax.text(x, y, str(number), ha="center", va="center", fontsize=14, color=BLACK, path_effects=[
            pe.withStroke(linewidth=3.2, foreground=BACKGROUND),
            pe.Normal(),
        ],)
...
def draw_proxy_box(ax, x, y, w: float = 2.4, h: float = 1.2):
    ...
    ax.add_patch(box)
    ax.text(x, y, "proxy", ha="center", va="center", fontsize=13, color=BLACK, path_effects=[
            pe.withStroke(linewidth=3.2, foreground=BACKGROUND),
            pe.Normal(),
        ],)
    return (x - w / 2, x + w / 2)
...
def panel_titles(ax: plt.Axes) -> None:
    ...
    ax.text(2.2, 8.5, "Swarm", ha="left", va="center", fontsize=18, color=BLACK, path_effects=[
            pe.withStroke(linewidth=3.2, foreground=BACKGROUND),
            pe.Normal(),
        ],)

    draw_circled_number(ax, 14.0, 8.5, 2)
    ax.text(15.2, 8.5, "Regression", ha="left", va="center", fontsize=18, color=BLACK, path_effects=[
            pe.withStroke(linewidth=3.2, foreground=BACKGROUND),
            pe.Normal(),
        ],)
    ...
    ax.text(27.7-df, 8.5, "Optimization", ha="left", va="center", fontsize=18, color=BLACK, path_effects=[
            pe.withStroke(linewidth=3.2, foreground=BACKGROUND),
            pe.Normal(),
        ],)
...
def draw_swarm_panel(ax: plt.Axes) -> None:
    ...
    ax.text(1.15, 3.8, r"$\vdots$", ha="center", va="center", fontsize=18, color=BLACK, path_effects=[
            pe.withStroke(linewidth=3.2, foreground=BACKGROUND),
            pe.Normal(),
        ],)
    ...
    ax.text(y_text_x, 5.3, r"$y_{11},\ldots,y_{n1}$",
            ha="left", va="center", fontsize=16, color=Y_COLOR, path_effects=[
            pe.withStroke(linewidth=3.2, foreground=BACKGROUND),
            pe.Normal(),
        ],)
    ...
    ax.text(0.2, 6.5, r"Mixes $p$", ha="left", va="center", fontsize=16, color=BLACK, path_effects=[
            pe.withStroke(linewidth=3.2, foreground=BACKGROUND),
            pe.Normal(),
        ],)
    ax.text(y_text_x, 6.5, "Performance", ha="left", va="center", fontsize=16, color=BLACK, path_effects=[
            pe.withStroke(linewidth=3.2, foreground=BACKGROUND),
            pe.Normal(),
        ],)
...
def draw_regression_panel(ax: plt.Axes) -> None:
    ...
    ax.plot([x0, x0], [y0, y0 + h], color=BLACK, lw=2.5, path_effects=[
            pe.withStroke(linewidth=3.2, foreground=BACKGROUND),
            pe.Normal(),
        ],)
    ax.plot([x0, x0 + w], [y0, y0], color=BLACK, lw=2.5, path_effects=[
            pe.withStroke(linewidth=3.2, foreground=BACKGROUND),
            pe.Normal(),
        ],)

    ax.plot([x0 + 0.6, x0 + w - 0.6], [y0 + 0.6, y0 + h - 0.6], color=FHAT_COLOR, lw=2.5, path_effects=[
            pe.withStroke(linewidth=3.2, foreground=BACKGROUND),
            pe.Normal(),
        ],)
    ...
    for px, py in pts:
        ax.plot(x0 + px, y0 + py, marker=".", markersize=6, color=Y_COLOR, lw=0, path_effects=[
            pe.withStroke(linewidth=3.2, foreground=BACKGROUND),
            pe.Normal(),
        ],)

    ax.text(x0 + w / 2, y0 - 1.0, r"Predicted $\hat{f}(p)$", ha="center", va="center", fontsize=16, color=FHAT_COLOR, path_effects=[
            pe.withStroke(linewidth=3.2, foreground=BACKGROUND),
            pe.Normal(),
        ],)
    ax.text(x0 - 1.0, y0 + h / 2, r"True $f(p)$", ha="center", va="center", rotation=90, fontsize=16, color=Y_COLOR, path_effects=[
            pe.withStroke(linewidth=3.2, foreground=BACKGROUND),
            pe.Normal(),
        ],)
...
def draw_math_segments(ax, x, y, segments, fontsize=20, va="center"):
    ...
    cur_x = x
    for s, c in segments:
        t = ax.text(cur_x, y, s, ha="left", va=va, fontsize=fontsize, color=c, path_effects=[
            pe.withStroke(linewidth=3.2, foreground=BACKGROUND),
            pe.Normal(),
        ],)
...
def place_icon(
    ...
    if icon_path and icon_path.exists():
        ...
        box = FancyBboxPatch(
            ...
            edgecolor=BLACK,
            linestyle="--",
            linewidth=1.5, path_effects=[
            pe.withStroke(linewidth=3.2, foreground=BACKGROUND),
            pe.Normal(),
        ],
        )
        ax.add_patch(box)
        ax.text(x, y, "ICON", ha="center", va="center", fontsize=9, color=BLACK, path_effects=[
            pe.withStroke(linewidth=3.2, foreground=BACKGROUND),
            pe.Normal(),
        ],)
...
def draw_optimization_panel(ax: plt.Axes) -> None:
    ...
    ax.plot(pstar_x, pstar_y-1, marker="o", markersize=5, color=BLACK, zorder=5, path_effects=[
            pe.withStroke(linewidth=3.2, foreground=BACKGROUND),
            pe.Normal(),
        ],)
    ax.text(pstar_x + 0.45, pstar_y-0.8, r"$p^*$", ha="left", va="center", fontsize=14, color=BLACK, path_effects=[
            pe.withStroke(linewidth=3.2, foreground=BACKGROUND),
            pe.Normal(),
        ],)
    ...
    ax.text(28.5-dx, 5.5, r"$p \in S$", ha="left", va="center", fontsize=16, color=BLACK, path_effects=[
            pe.withStroke(linewidth=3.2, foreground=BACKGROUND),
            pe.Normal(),
        ],)

    ax.text(pstar_x + 4, pstar_y-0.5, r"$\hat{f}$", ha="left", va="center", fontsize=14, color=FHAT_COLOR, path_effects=[
            pe.withStroke(linewidth=3.2, foreground=BACKGROUND),
            pe.Normal(),
        ],)
